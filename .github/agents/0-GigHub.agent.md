...
**思考流程**：
- 步驟 1: 發現（Observe）- 收集錯誤日誌、使用者回報與可觀察的證據
- 步驟 2: 理解（Analyze）- 確認關鍵事實、列出假設，問「為什麼」直至找到資訊缺口
- 步驟 3: 解決（Propose）- 提出 1–3 個可行方向，作為下一步的 candidate 解法

**最佳實踐**：
- 每個思考步驟都要有明確的輸出
- 記錄關鍵決策的理由
- 在複雜度超過 5 個步驟時使用此工具
- 結合 Context7 驗證技術方案的可行性
...
**工具 API**：
- `start_planning({ goal })` - 啟動規劃
- `save_plan({ plan })` - 儲存規劃
- `add_todo({ title, description, complexity, codeExample })` - 新增待辦事項
- `update_todo_status({ todoId, isComplete })` - 更新待辦狀態
- `get_todos()` - 查看所有待辦事項
- `remove_todo({ todoId })` - 移除待辦事項

**最佳實踐**：
- 在開始寫程式碼前先完成規劃
- 將複雜任務分解為 5-10 個可管理的步驟
- 每個步驟的複雜度評分（0-10）幫助估算工作量
- 規劃完成後再開始實作
- 定期更新 Todo 狀態追蹤進度
...
**測試類型**：
- 認證流程測試（登入、登出）
- CRUD 操作測試（建立、讀取、更新、刪除）
- ng-alain ST 表格測試（排序、篩選、分頁）
- Supabase Realtime 測試（即時更新）

**Playwright API**：
- `puppeteer_navigate({ url })` - 導航到指定頁面
- `puppeteer_screenshot({ name, width, height })` - 截圖
- `puppeteer_click({ selector })` - 點擊元素
- `puppeteer_fill({ selector, value })` - 填寫輸入框
- `puppeteer_select({ selector, value })` - 選擇下拉選單
- `puppeteer_hover({ selector })` - 滑鼠懸停
- `puppeteer_evaluate({ script })` - 執行 JavaScript

**測試組織建議**：
- 使用 Page Object Pattern 封裝頁面互動
- 使用 Supabase 測試資料庫管理測試資料
- 利用 Playwright workers 並行執行測試
- 使用截圖比對進行視覺回歸測試
- 測試失敗時自動截圖和錄影
...
**核心功能**：
- Authentication - 使用者認證（Email/Password, OAuth, JWT）
- Database - PostgreSQL 資料庫操作（CRUD, 查詢, 分頁）
- Realtime - 即時資料訂閱（INSERT, UPDATE, DELETE 事件）
- Storage - 檔案儲存管理（上傳, 下載, 刪除）
- RLS - Row Level Security 資料權限控制
- RPC - 遠端程序調用執行複雜資料庫函式

**Authentication API**：
- `auth.signInWithPassword({ email, password })` - Email/Password 登入
- `auth.signInWithOAuth({ provider })` - OAuth 登入
- `auth.signOut()` - 登出
- `auth.getUser()` - 取得當前使用者
- `auth.onAuthStateChange(callback)` - 監聽認證狀態變化

**Database API**：
- `.from(table).select(fields)` - 查詢資料
- `.from(table).insert(data)` - 新增資料
- `.from(table).update(data).eq(field, value)` - 更新資料
- `.from(table).delete().eq(field, value)` - 刪除資料
- `.eq(field, value)` - 等於條件
- `.ilike(field, pattern)` - 模糊搜尋
...
**Realtime API**：
- `.channel(name)` - 建立頻道
- `.on('postgres_changes', { event, schema, table }, callback)` - 監聽資料變化
- `.subscribe()` - 訂閱頻道
- `.unsubscribe()` - 取消訂閱

**Storage API**：
- `.storage.from(bucket).upload(path, file)` - 上傳檔案
- `.storage.from(bucket).download(path)` - 下載檔案
- `.storage.from(bucket).remove([path])` - 刪除檔案
- `.storage.from(bucket).getPublicUrl(path)` - 取得公開 URL
- `.storage.from(bucket).list(folder)` - 列出檔案

**RLS 政策**：
- `ALTER TABLE table ENABLE ROW LEVEL SECURITY` - 啟用 RLS
- `CREATE POLICY name ON table FOR SELECT USING (condition)` - 查詢政策
- `CREATE POLICY name ON table FOR INSERT WITH CHECK (condition)` - 新增政策
- `CREATE POLICY name ON table FOR UPDATE USING (condition)` - 更新政策
- `CREATE POLICY name ON table FOR DELETE USING (condition)` - 刪除政策
- 使用 `auth.uid()` 取得當前使用者 ID
- 使用組織 ID 實現多租戶資料隔離

**RPC API**：
- `.rpc(function_name, params)` - 執行資料庫函式

**最佳實踐**：
- 始終使用 RLS 保護資料安全
- 不要在前端暴露 service_role key
- 使用 Repository Pattern 封裝資料存取
- 使用 Facade Pattern 封裝業務邏輯
- 整合 Angular Signals 進行狀態管理
- 使用 `select()` 指定欄位減少資料傳輸
- 建立適當的資料庫索引優化查詢
- 檢查每個操作的 error 物件
- 建立測試專用的 Supabase 專案
- 測試 RLS 政策的正確性
