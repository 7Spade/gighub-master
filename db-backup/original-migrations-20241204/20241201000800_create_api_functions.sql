-- ============================================================================
-- Migration: Create API Functions
...
BEGIN
  ...
  IF p_slug IS NULL OR p_slug = '' THEN
    v_slug := lower(regexp_replace(p_name, '[^a-zA-Z0-9]+', '-', 'g'));
    v_slug := trim(both '-' from v_slug);
    WHILE EXISTS (SELECT 1 FROM public.blueprints WHERE owner_id = p_owner_id AND slug = v_slug) LOOP
      v_slug := v_slug || '-' || substr(gen_random_uuid()::text, 1, 8);
    END LOOP;
  ELSE
    v_slug := p_slug;
  ...
  -- 6. 將建立者加入藍圖成員 (role=maintainer)
  INSERT INTO public.blueprint_members (blueprint_id, account_id, role, business_role)
  VALUES (v_blueprint_id, v_user_account_id, 'maintainer', 'project_manager')
...
BEGIN
  IF NEW.created_by IS NOT NULL THEN
    ...
    IF v_account_id IS NOT NULL THEN
      INSERT INTO public.blueprint_members (blueprint_id, account_id, role, business_role)
      VALUES (NEW.id, v_account_id, 'maintainer', 'project_manager')
      ON CONFLICT (blueprint_id, account_id) DO NOTHING;
    END IF;
  END IF;
  RETURN NEW;
...
-- DOWN (Rollback)
-- ============================================================================
-- DROP TRIGGER IF EXISTS on_blueprint_created ON blueprints;
-- DROP TRIGGER IF EXISTS on_organization_created ON organizations;
-- DROP FUNCTION IF EXISTS public.create_blueprint(UUID, VARCHAR, VARCHAR, TEXT, TEXT, BOOLEAN, public.module_type[]);
-- DROP FUNCTION IF EXISTS public.handle_new_blueprint();
-- DROP FUNCTION IF EXISTS public.create_team(UUID, VARCHAR, TEXT, JSONB);
