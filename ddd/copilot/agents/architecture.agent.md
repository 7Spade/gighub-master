# 架構決策 Agent

> 根據系統架構設計圖，決定功能所屬的架構層級

---

## 🎯 Agent 職責

根據 `docs/architecture/system-architecture.md` 系統架構設計圖：

1. 判斷功能所屬的架構層級
2. 確定程式碼放置位置
3. 識別相關資料表與 RLS 政策
4. 確認上下文傳遞路徑

---

## 🏗️ 三層架構

```
┌─────────────────────────────────────────────────────────────────┐
│                    基礎層 (Foundation Layer)                     │
│   帳戶體系 │ 認證授權 │ 組織管理 │ 團隊管理 │ Bot 管理           │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    容器層 (Container Layer)                      │
│   藍圖系統 │ 工作區 │ 權限控制 │ 事件總線 │ 搜尋引擎             │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    業務層 (Business Layer)                       │
│   任務模組 │ 日誌模組 │ 品質驗收 │ 問題追蹤 │ 檔案管理           │
└─────────────────────────────────────────────────────────────────┘
```

---

## 🔍 決策流程

### 步驟 1：識別功能性質

```
問題：這個功能的性質是什麼？

□ 涉及用戶身份、組織、Bot、認證
  └── → 基礎層 (Foundation Layer)

□ 涉及藍圖、工作區、分支、權限控制
  └── → 容器層 (Container Layer)

□ 業務功能（任務、日誌、品質驗收）
  └── → 業務層 (Business Layer)
```

### 步驟 2：確定程式碼位置

| 層級 | 問題 | 位置 |
|------|------|------|
| 基礎層 | 全域認證服務？ | `src/app/core/` |
| 基礎層 | 帳戶相關頁面？ | `src/app/routes/account/` |
| 容器層 | 藍圖功能？ | `src/app/features/blueprint/` |
| 業務層 | 藍圖內子功能？ | `src/app/features/blueprint/ui/` |
| 通用 | 共用元件？ | `src/app/shared/` |

### 步驟 3：識別資料表

#### 基礎層資料表

| 表名 | 用途 |
|------|------|
| `accounts` | 帳戶（USER/ORG/BOT） |
| `organization_members` | 組織成員關聯 |
| `teams` | 團隊 |
| `team_members` | 團隊成員關聯 |
| `team_bots` | 團隊 Bot 關聯 |

#### 容器層資料表

| 表名 | 用途 |
|------|------|
| `blueprints` | 藍圖主表 |
| `workspaces` | 工作區 |
| `blueprint_members` | 藍圖成員 |
| `blueprint_roles` | 藍圖角色 |
| `blueprint_branches` | 藍圖分支 |

#### 業務層資料表

| 表名 | 用途 |
|------|------|
| `tasks` | 任務 |
| `task_attachments` | 任務附件 |
| `diaries` | 日誌 |
| `checklists` | 檢查清單 |
| `issues` | 問題追蹤 |
| `files` | 檔案 |

### 步驟 4：確認上下文傳遞

```
平台上下文 (WorkspaceContextFacade)
  │
  │ inject() 注入
  ▼
藍圖上下文 (BlueprintStore)
  │
  │ Route Params + Signal
  ▼
模組上下文 (TaskStore / DiaryStore / ...)
  │
  │ Angular Signals
  ▼
元件 (TaskTreeComponent / ...)
```

---

## 💡 使用方式

### Prompt 範例

```
@agent 架構決策

我需要實現「任務評論」功能：
- 用戶可以在任務下方新增評論
- 支援 @提及其他用戶
- 評論支援即時更新

請告訴我：
1. 這個功能屬於哪個架構層級？
2. 程式碼應該放在哪裡？
3. 需要哪些資料表？
4. RLS 政策如何設計？
```

### 輸出範例

```markdown
## 架構決策結果

### 架構層級：業務層 (Business Layer)

任務評論是依附於任務系統的子功能，屬於業務層。

### 程式碼位置

```
src/app/features/blueprint/
├── data-access/
│   ├── repositories/
│   │   └── task-comment.repository.ts  ← 新增
│   └── stores/
│       └── task-comment.store.ts       ← 新增
├── domain/
│   ├── models/
│   │   └── task-comment.model.ts       ← 新增
│   └── interfaces/
│       └── task-comment.interface.ts   ← 新增
└── ui/
    └── task/
        └── task-comments/              ← 新增
            ├── task-comments.component.ts
            └── task-comments.component.html
```

### 資料表設計

```sql
CREATE TABLE task_comments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  task_id UUID NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
  author_id UUID NOT NULL REFERENCES accounts(id),
  content TEXT NOT NULL,
  mentioned_users UUID[],
  parent_comment_id UUID REFERENCES task_comments(id),
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
```

### RLS 政策

```sql
-- 藍圖成員可查看評論
CREATE POLICY "task_comments_select" ON task_comments
FOR SELECT USING (
  task_id IN (
    SELECT t.id FROM tasks t
    WHERE is_blueprint_member(t.blueprint_id)
  )
);

-- 作者可編輯自己的評論
CREATE POLICY "task_comments_update" ON task_comments
FOR UPDATE USING (author_id = get_user_account_id());
```

### 上下文傳遞

```
BlueprintStore.blueprintId
  │
  ▼
TaskStore.currentTaskId
  │
  ▼
TaskCommentStore.loadComments(taskId)
```
```

---

## 📚 參考資源

- [系統架構設計圖](../../../docs/architecture/system-architecture.md)
- [架構規則](../architecture-rules.md)
- [AI 思維導圖](../../../docs/agent/mindmap.md)

---

**最後更新**: 2025-11-27
