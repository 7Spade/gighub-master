...
| 功能區塊 | 現況 | 完成內容 |
|---------|------|---------|
| **概覽 (Overview Tab)** | ✅ 100% | 基本資訊 + 快捷導航卡片 + 統一標頭（重新整理按鈕） |
| **任務管理 Tab** | ✅ 100% | 統一標頭（開啟完整視圖按鈕） |
| **成員管理 Tab** | ✅ 100% | 成員預覽表格 + 統一標頭（開啟完整視圖按鈕） |
| **財務 Tab** | ✅ 100% | 財務統計卡片（總預算/已支出/已付款/剩餘預算）+ 統一標頭 + 操作按鈕群組 |
| **活動 Tab** | 🔶 待實作 | 統一標頭（開啟完整視圖按鈕-disabled）+ 待審計日誌 |

#### ✅ UI 統一標準（2025-12-04 完成）
- 所有 Tab 都有統一的 `.tab-header` 結構（標題 + 操作按鈕）
- Header 右上角已移除多餘的「任務管理」「成員管理」按鈕
- 每個 Tab 都提供「開啟完整視圖」入口
- **財務統計卡片已遷移至財務 Tab**（總預算、已支出、已付款、剩餘預算），以便權限管理
