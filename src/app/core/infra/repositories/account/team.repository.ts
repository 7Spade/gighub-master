...
export class TeamRepository {
  ...
  findByOrganization(organizationId: string): Observable<Team[]> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) {
          console.error('[TeamRepository] findByOrganization error:', error);
          return [];
        }
        return (data || []) as Team[];
      })
  ...
  update(id: string, updates: Partial<Team>): Observable<Team | null> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) {
          console.error('[TeamRepository] update error:', error);
          return null;
        }
        return data as Team;
      })
  ...
  softDelete(id: string): Observable<Team | null> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) {
          console.error('[TeamRepository] softDelete error:', error);
          return null;
        }
        return data as Team;
      })
  ...
  updateMemberRole(id: string, role: TeamRole): Observable<TeamMember | null> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) {
          console.error('[TeamRepository] updateMemberRole error:', error);
          return null;
        }
        return data as TeamMember;
      })
